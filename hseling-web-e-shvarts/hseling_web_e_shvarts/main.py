...
if os.environ.get('HSELING_WEB_E_SHVARTS_SETTINGS'):
    app.config.from_envvar('HSELING_WEB_E_SHVARTS_SETTINGS')
...
if not app.debug:
    ...
    file_handler = TimedRotatingFileHandler(os.path.join(app.config['LOG_DIR'], 'hseling_web_e_shvarts.log'), 'midnight')
...
def healthz():
    ...
    return jsonify({"status": "ok", "message": "hseling-web-e-shvarts"})

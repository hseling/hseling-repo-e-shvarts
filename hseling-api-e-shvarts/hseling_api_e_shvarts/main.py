...
from hseling_api_e_shvarts import boilerplate

from hseling_lib_e_shvarts.process import process_data
from hseling_lib_e_shvarts.query import query_data


app = Flask(__name__)
app.config['DEBUG'] = False
app.config['LOG_DIR'] = '/tmp/'
if os.environ.get('HSELING_API_E_SHVARTS_SETTINGS'):
    app.config.from_envvar('HSELING_API_E_SHVARTS_SETTINGS')
...
if not app.debug:
    ...
    file_handler = TimedRotatingFileHandler(os.path.join(app.config['LOG_DIR'], 'hseling_api_e_shvarts.log'), 'midnight')
...
def healthz():
    ...
    return jsonify({"status": "ok", "message": "hseling-api-e-shvarts"})
